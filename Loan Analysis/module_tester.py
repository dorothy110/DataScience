...
def loans_test():
    ...
    # my test
        # __lt__
    applicants = sorted([
        loans.Applicant(">60", ["43", "44"]),
        loans.Applicant("30-40", ["1", "3"]),
        loans.Applicant("23-30", ["22"]),
        loans.Applicant("<22", ["5"]),
    ])
    assert [a.lower_age() for a in applicants] == [22, 23, 30, 60]
    loans_points += 1
    

    # repr
    applicant = loans.Applicant("90-100", [])
    assert repr(applicant) == "Applicant('90-100', [])"
    applicant = loans.Applicant("100+", ["25"])
    assert repr(applicant) == "Applicant('100+', ['Korean'])"
    loans_points += 1


    # lower_age
    assert loans.Applicant("<26", []).lower_age() == 26
    assert loans.Applicant("27-40", []).lower_age() == 27
    assert loans.Applicant(">60", []).lower_age() == 60
    loans_points += 1  
    
    # init numeric fields
    b = {"loan_amount": "222", "property_value": "500", "interest_rate": "4.6",
    ...
    loan = loans.Loan(b)
    assert loan.loan_amount == 222
    assert loan.property_value == 500
    assert loan.interest_rate == 4.6
    loans_points += 1
